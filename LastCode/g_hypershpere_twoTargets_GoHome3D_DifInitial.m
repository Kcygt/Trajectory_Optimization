...
xCtrl = zeros(2,3);
qCtrl = zeros(2,3);
xTarget(1,:) = [-0.15, -0.05, 0.0];
xTarget(2,:) = [ 0.15, -0.05, 0.0];
...
qInitial = [0, 0.2422, 0.0362];
x0 = [zeros(1,6), qInitial, zeros(1,3)];  % 6 zeros for prefilter, 3 for q, 3 for qd
[tInit, yInit] = ode45(@(t, x) myTwolinkwithprefilter(t, x, qDes, tspan, wn1, wn2, wn3, xCtrl(1,:), xCtrl(2,:)), t_uniform, x0);
...
function [c, ceq] = trajConstraint(prms,qDes,xTarget,xFinal)
    ...
    % End point error
    distEndErr = sum((x(end,:) - xFinal).^2,2);
