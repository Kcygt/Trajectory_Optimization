...
% Weights
wt = [800, 5, 0.001]; % [Target, End, Time]

initPrms = [tspan, wn];

tUni = 0:0.001: tspan(end);
% Initial Condition
[tInit, yInit] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, tspan, wn,xTarget),tUni, zeros(12, 1));
...
lb = [0 0 0  ... % time
      1 0.5 0.5  1 0.5 0.5  1 0.5 0.5 ]; % Wn
ub = [2 2 2 ... % time
      1  30  30   1  30  30   1  30  30]; % Wn
...
[tOpt, yOpt] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, Opt(1:3),  Opt(4:12),xTarget), ...
                 tUni, zeros(12, 1));
...
function error = objectiveFunction(prms, qDes, wt, xTarget, xFinal)
    ...
    tUni = 0:0.001:prms(3);
    % Simulate the system
    [~, y] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes,prms(1:3),prms(4:12),xTarget), ...
                    tUni, x0);
...
function [c, ceq] = trajConstraint(prms,qDes,xTarget)
    ...
    % Simulate trajectory
    [~, yy] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes,prms(1:3),prms(4:12),xTarget), ...
                    tUni, zeros(12, 1));
...
function dxdt= myTwolinkwithprefilter(t,x,qDes,t_st,wn,xTarget)
    zeta = [1 1 1];
    ...
    qTarget1 = IK(xTarget(1,1),xTarget(1,2),xTarget(1,3));
    qTarget2 = IK(xTarget(2,1),xTarget(2,2),xTarget(2,3));
    ...
    qdd=M\(tau-C*qd);
    if t <= t_st(1)
        dxdt=[A1*x(1:6)+B1*qTarget1'; qd; qdd];
    elseif t > t_st(1) && t <= t_st(2)
        dxdt=[A2*x(1:6)+B2*qTarget2'; qd; qdd];
    else
