...
Kd = [20 20 20];
% Weights
wt = [50, 1, 0.08, 1, 1]; % [Target, End, Time]
...
% Objective Function
function error = objectiveFunction(Opt, qDes, wt, xMid, xDes)
    x0 = zeros(12, 1);
    x0(1:3) = qDes;

    % Simulate the system
    [tt, y] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes, Opt(1),  Opt(2:4), Opt(5:7),Opt(8:10),Opt(11:13)), ...
                    [0 Opt(1)], x0);
    ...
    VelErr = y(:,10:12) - y(:,4:6); 
    
    e = sum(mean(abs(PosErr)),2);
    edot = sum(mean(abs(VelErr)),2);

    % Composite error (normalized)
    error = wt(1)*distMid + wt(2)*distEndErr + wt(3)*timePenalty + wt(4) * e + wt(5) * edot;
end

% Constraint Function for Midpoint Proximity
function [c, ceq] = trajConstraint(Opt,qDes,xMid)
    ceq = []; % No equality constraints

    % Simulate trajectory
    [~, yy] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes,Opt(1),  Opt(2:4), Opt(5:7),Opt(8:10),Opt(11:13)), ...
                    [0 Opt(1)], zeros(12, 1));
