...
qTarget = zeros(2,3);
qTarget(1,:) = IK(xTarget(1,1),xTarget(1,2),xTarget(1,3));
qTarget(2,:) = IK(xTarget(2,1),xTarget(2,2),xTarget(2,3));

tspan = [5 10 15];
wn = 2*[1 1 1   1 1 1    1 1 1];
...
% Initial Condition
[tInit, yInit] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, tspan, wn,qTarget),tUni, zeros(12, 1));
...
function dxdt= myTwolinkwithprefilter(t,x,qDes,t_st,wn,qTarget)
    zeta = [1 1 1];
    ...
    qdd=M\(tau-C*qd);
    if t <= t_st(1)
        dxdt=[A1*x(1:6)+B1*qTarget(1,:)'; qd; qdd];
    elseif t > t_st(1) && t <= t_st(2)
        dxdt=[A2*x(1:6)+B2*qTarget(2,:)'; qd; qdd];
    else
