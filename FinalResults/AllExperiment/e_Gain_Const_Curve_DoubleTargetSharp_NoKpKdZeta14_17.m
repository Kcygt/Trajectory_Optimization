...
% Data 14
tspan = [0.576192     0.895692      1.73493];
wn = [4.49918  3.0787  1.57671 4.49918 1.50777  29.3524 4.49918       5.8954      2.68569 ];

% Data 15
% tspan = [0.242992      1.77835      1.79007];
% wn = [  1      3.32578      5.12966  1       19.658      3.07353  1      18.8954      25.8191 ];

% Data 16
% tspan = [0.432502     0.966013       1.5413 ];
% wn = [1       15.185      13.2658 1       11.431      25.6703 1       7.3239      9.16861 ];

% % Data 17
% tspan = [ 0.51225      1.2425      1.9047 ];
% wn = [   1      5.6425       2.886  1      3.6187      4.8392  1      3.1022      2.6144 ];

Gain = 5;
tspan1 = tspan /Gain;
wn1 = wn * Gain;

dataNum = 14;  % Change this to 2, 3, etc. for other runs

tUni = 0:0.001: tspan(end);
tUni1 = 0:0.001: tspan1(end);
% Initial Condition
[tInit, yInit] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, tspan, wn),tUni, zeros(12, 1));
[tInit1, yInit1] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, tspan1, wn1),tUni1, zeros(12, 1));

%%% Plotting
[CxInit, CyInit, CzInit] = FK(yInit(:,7), yInit(:,8), yInit(:,9)); % Initial Trajectory
[CxOpt, CyOpt, CzOpt] = FK(yInit1(:,7), yInit1(:,8), yInit1(:,9)); % Optimized Trajectory
