...
function error = objectiveFunction(prms, qDes, wt, xMid, xDes)
    ...
    % Simulate the system
    [t, y] = ode23s(@(t,x) myTwolinkwithprefilter(t, x, qDes, prms(1),  prms(2:4), prms(5:7)), ...
                    t_uniform, x0);

    [xOut,yOut,zOut] = FK(y(:,7),y(:,8),y(:,9));
    xOut = [xOut,yOut,zOut];
    
    squaredDist1  =  min(sum((xOut - xMid(1,:)).^2,2))  ;
...
function [c, ceq] = trajConstraint(prms,qDes,xTarget)
    ...
    t_uniform = 0:0.01:T_total;
    % Simulate trajectory
    [tt, yy] = ode23s(@(t,x) myTwolinkwithprefilter(t, x, qDes, prms(1),  prms(2:4), prms(5:7)), ...
                    t_uniform, zeros(12, 1));
    [x,y,z] = FK(yy(:,7),yy(:,8),yy(:,9));     % Optimized Trajectory
    x = [x,y,z];
    
    squaredDist1  =  min(sum((x - xTarget).^2,2)) ;
