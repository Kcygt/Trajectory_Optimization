...
wt = [10, 1e-4, 20];    % Weights [qDes, Time, qMid]
% wt = [10, 1e-4, 500];  % Weights [qDes, Time, qMid]

% Optimization setup
initParams = [time zeta wn bj kj]; % Initial guess

[init_T, init_Y] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, zeta, wn, time, qDes, bj, kj), [0 time], zeros(12, 1));
[xInit, yInit, zInit] = FK(init_Y(:,7), init_Y(:,8), init_Y(:,9));
[xD, yD, zD] = FK(init_Y(:,1), init_Y(:,2), init_Y(:,3));

% plot(xInit,zInit,'+',xD,zD,'.')
% Lower and upper boundaries 
lb = [3   0.5 0.8 0.6  1  1  1   10 10 10  70 70 70];   
...
[t, y] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, optimalParams(2:4), optimalParams(5:7), optimalParams(1), qDes, optimalParams(8:10), optimalParams(11:13)), [0 optimalParams(1)], zeros(12, 1));
...
function error = objectiveFunction(params, qDes, wt, xMid)
    ...
    distMid1 = min(sum(  (xAct - xMid(1, :)).^2  , 2));
    distMid2 = min(sum(  (xAct - xMid(2, :)).^2  , 2));
    distMid3 = min(sum(  (xAct - xMid(3, :)).^2  , 2));

    error = wt(1) * distto1   +  ...
            wt(2) * params(1) + ... 
            wt(3) * (distMid1+distMid2+distMid3);
end
