...
%% ===== ADD 3D RECTANGULAR BOX =====
%% ===== ADD BUMP SURFACE ALONG Y =====
Lx = 0.15;  % length along X
Ly = 0.04;  % length along Y
Lz = 0.04;  % height along Z
center = [0.02, -0.04, 0];

% Mesh resolution
nx = 40; nz = 20;
[xg, zg] = meshgrid(linspace(-Lx/2, Lx/2, nx), linspace(-Lz/2, Lz/2, nz));

% Gaussian bump along Y (sideways bulge)
bumpMax = Ly/2; % maximum half-width in Y
sigma = Lx/4;   % spread of bump along X axis
yg = bumpMax * exp(-(xg/sigma).^2);
