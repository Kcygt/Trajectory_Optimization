...
Kp_list = {diag([3 3 3]), diag([50 50 50]), diag([50 50 50])};
Kd_list = {diag([1  1 1]), diag([1 1 1]), diag([60 60 60])};
...
for i = 1:length(Kp_list)
    ...
    [CxDes, CyDes, CzDes] = FKnew(ySim(:,1), ySim(:,2), ySim(:,3));
...
title(sprintf('Cartesian Trajectory (YZ plane)'));

colors = lines(length(results));   % MATLAB palette
legendEntries = {};
for i = 1:length(results)
    plot(results{i}.Cy, results{i}.Cz, 'Color', colors(i,:), 'LineWidth', 2);
    legendEntries{end+1} = sprintf('Kp = [ %d, %d ], Kd = [ %d, %d ]', ...
        results{i}.Kp(2:3), results{i}.Kd(2:3));
end
plot(0,0, 's', 'LineWidth',2, 'MarkerSize', 10, 'Color', [1,0.5,0.05]);
legendEntries{end+1} = 'Start Point';

plot(xFinal(2), xFinal(3), 'o', 'LineWidth',2,'MarkerSize', 10, 'Color', [.2,1,0.5]);
legendEntries{end+1} = 'Final Point';

plot(CyDes,CzDes, 'k--', 'LineWidth', 2)
legendEntries{end+1} = 'Reference Cartesian Trajectory';
...
for i = 1:length(results)
    figure(1+i);
    sgtitle( sprintf('Joint Position (K_p = [ %d, %d ], K_d = [ %d, %d ])', ...
        results{i}.Kp(2:3), results{i}.Kd(2:3)), ...
    ...
    for j = 2:3
        ...
        text(xPos-.5, yPos, sprintf('RMSE = %.4f (rad)', rmse_val), ...
            'FontSize', 10, 'Color', colors(i,:), 'FontWeight','bold');

        % 👇 Add correct title per joint
...
for i = 1:length(results)
    ...
    sgtitle(sprintf('Joint Velocity (K_p = [%d,%d], K_d = [%d,%d])', ...
        results{i}.Kp(2:3), results{i}.Kd(2:3)));
    ...
    for j = 2:3
        ...
        xlabel('Time (s)');
        ylabel(sprintf('Velocity  (rad/s)'));
        legend('Actual','Reference','Location','best');

        % === Add RMSE as text on the plot ===
        rmse_val = results{i}.rmse_qd(j);
        xVel = results{i}.t(end) * 0.7;   
        yVel = mean([min(results{i}.qd_actual(:,j)) max(results{i}.qd_actual(:,j))]);
   
        text(xVel-.5, yVel+0.06, sprintf('RMSE = %.4f (rad/s)', rmse_val), ...
            'FontSize', 10, 'Color', colors(i,:), 'FontWeight','bold');
...
%% ===== Save All Figures as PNG and FIG =====
outputFolder = 'results';  % folder to store results
if ~exist(outputFolder, 'dir')
    mkdir(outputFolder);
end

numFigs = length(findobj('Type', 'figure'));  % count open figures
for f = 1:numFigs
    figure(f);
    pngFile = fullfile(outputFolder, sprintf('figure_%d.png', f));
    figFile = fullfile(outputFolder, sprintf('figure_%d.fig', f));
