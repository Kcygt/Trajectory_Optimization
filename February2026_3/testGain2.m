...
%% ================= COLORS =================
colorSim      = [0.0 0.8 0.9];     % Reference trajectory
colorRef      = [0 0 0];
homeColor     = [0 0 0];
...
targetLegendLabels = {
    'Force on Target points (normal speed)'
    'Force on Target points (2x speed)'
    'Force on Target points (4x speed)'
};
...
hProj3D    = gobjects(numData,1);
hAct2D     = gobjects(numData,1);
hProj2D    = gobjects(numData,1);
...
for dataNum = startData:endData
    ...
    %% ===== Continuous Force-Based Projection =====
    yProj = yDes + (Fdes - Fz) ./ k;
    ...
    hProj3D(idx) = plot3(xDes,yProj,zDes,...
        '--','Color',trajColors{idx},'LineWidth',2);
    ...
    hAct2D(idx) = plot(xAct,yAct,...
        'Color',trajColors{idx},'LineWidth',2);

    hProj2D(idx) = plot(xDes,yProj,...
        '--','Color',trajColors{idx},'LineWidth',2);
...
figure(2)
legend([hDes; hAct3D; hProj3D; hHome; hCtrl; hInit; hRegion], ...
       [{'Reference Trajectory'}; ...
        strcat(trajLabels,' (measured)'); ...
        strcat(trajLabels,' (projected)'); ...
        {'Home';'Control Points';'Target Points';'Control Point Sphere'}], ...
       'Location','best')

figure(3)
legend([hHome2; hDes2; hAct2D; hProj2D; hInit2; hCtrl2; hRegion2], ...
       [{'Home';'Reference Trajectory'}; ...
        strcat(trajLabels,' (measured)'); ...
        strcat(trajLabels,' (projected)'); ...
        {'Target Points';'Control Points';'Control Point Sphere'}], ...
       'Location','best')
