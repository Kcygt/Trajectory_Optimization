...
%         [x_d_curr_z, ~, gamma_d(:, 3)] = optimalDeformation(delta, mu, gamma_d(:, 3), 0.0, H, tau_f);
        [x_d_curr_z, ~, gamma_d_z] = optimalDeformation2(x_d_star, loopCounter, N, delta, T, gamma_d_z, f_err(3), H, mu);
...
save 'output_1.mat'
plotExperimentalData(listOfCSVFiles(end).name, 'output_1.mat')
