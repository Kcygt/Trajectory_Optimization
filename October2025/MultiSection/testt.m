...
% Preallocate result
xTarget = [0.0208827243478188	0.0162800519366574	0.0249648836518187;
           0.0419206278166634	0.0692949060618982	0.0938418637779708];
...
tspan = [0.505685118729296	0.922621264994704	1.75060131737101] / Gain;
wn = Gain * [1.48798391877174	1.78862620712836	0.637462555971383 ...
          2.85119531642545	5.59527274103035	7.98971410740924 ...
      	  0.391227682189937	6.26746771171851	5.01774403158234];
...
palette = [
    ...
    0.8000 0.6000 0.7000;  % pink
    0.9500 0.9000 0.9500;  % light violet
    0.6350 0.0780 0.1840;  % dark red

];

% Plot each segment based on tspan
for i = 1:length(tspan)
    if i == 1
        idx = find(tInit <= tspan(i));
    else
        idx = find(tInit > tspan(i-1) & tInit <= tspan(i));
    end

    plot3(CxInit(idx), CyInit(idx), CzInit(idx), ...
        'Color', palette(i,:), 'LineWidth', 2);
end

% Plot the remaining part (after tspan(end), if any)
if tInit(end) > tspan(end)
    idx = find(tInit > tspan(end));
    plot3(CxInit(idx), CyInit(idx), CzInit(idx), ...
...
plot3(xTarget(1,1), xTarget(1,2), xTarget(1,3), '*', ...
      'MarkerEdgeColor', palette(8,:), 'MarkerFaceColor', palette(7,:), ...
...
legend('0–t1', 't1–t2', 't2–t3', 'Target 1', 'Target 2');
title('3D Trajectory by Time Segments (Color Palette)');
view(3);
rotate3d on;
