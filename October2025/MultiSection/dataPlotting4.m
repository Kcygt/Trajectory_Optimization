...
function plotPhantomSimulation(Pdata, Sdata, Gdata, figPrefix)
    ...
    if ~isempty(xTarget)
        ...
        sep = [repmat('-',1,108) '\n'];
        fprintf(hdr, 'Target', 'Reference', 'Simulation', 'Hardware', 'Gain');
        fprintf(sep);
    ...
    series(2).name = 'Hardware';    series(2).t = Ptime; series(2).q = PqAct; series(2).qd = PqdAct;
    ...
    if hasPdata || hasSdata || hasGdata
        ...
        for j = 1:3
            ...
            if ~isempty(PqdAct)
                ...
                subplotHandles(end+1) = hP;
                subplotLabels{end+1}  = 'Hardware ($\dot{q}_{hw}$)';
            end
...
function [hOrdered, lOrdered] = orderLegend(h, l)
    ...
    desired = {'Simulation','Hardware','Reference'};
    idx = [];
