...
for dataNum = startData:endData
    ...
    legend([hDes hAct hHome hCtrl hInit hUpd hRegion], ...
           {'Desired Trajectory','Actual Trajectory','Home','Control Points', ...
            'Initial Target','Updated Target','Control Region'}, 'Location','best')
    ...
    % Legend
    legend([hDes2 hAct2 hInit2 hUpd2 hCtrl2 hRegion2], ...
           {'Desired Trajectory','Actual Trajectory','Initial Target','Updated Target','Control Points','Control Region'}, ...
           'Location','best')

    axis equal
    set(gca,'FontSize',12,'LineWidth',1.2)
    title(['Y vs X Trajectory for Dataset ', num2str(dataNum)])
    xlabel('X'); ylabel('Y')
