...
% Initial Condition
[ti, yi] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, tspan,  zeta,wn,Kp,Kd), [0 tspan], zeros(12, 1));
...
function error = objectiveFunction(prms, qDes, wt, xMid, xDes)
    ...
    % Simulate the system
    [~, y] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes,prms(1),prms(2:4),prms(5:7),prms(8:10),prms(11:13)), ...
                    [0 prms(1)], x0);
...
function [c, ceq] = trajConstraint(prms,qDes,xMid)
    ...
    % Simulate trajectory
    [~, yy] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes,prms(1),prms(2:4),prms(5:7),prms(8:10),prms(11:13)), ...
                    [0 prms(1)], zeros(12, 1));
