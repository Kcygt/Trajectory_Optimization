...
xTrgt = zeros(3,3);
xTrgt(1,:) = [ 0, 0.015, 0.005];
xTrgt(2,:) = [ 0, 0.03,  0.02];
xTrgt(3,:) = [ 0, 0.045, 0.035];
...
lb = [0 0  ...         % time switching and end
      0.5 0.5 0.5 ...  % wn1
      0.5 0.5 0.5 ...  % wn2 
      0.01   0.01   0.01];      % Control point

ub = [5 5 ...          % time switching and end
      20 20 20 ...     % wn1
      20 20 20 ...     % wn2 
      0.05 0.05 0.05]; % Control point


% Objective Function
objectiveFunc = @(params) objectiveFunction(params, qDes, wt, xTrgt, xDes);
...
problem = createOptimProblem('fmincon',...
    ...
    'ub', ub, ...
    'options', options, ...
    'nonlcon', @(prms) trajConstraint(prms, qDes, xTrgt));
...
plot(xi, zi,'--')
plot(x_opt,z_opt,'.-')
plot(xTrgt(1,1),xTrgt(1,3),'*')
plot(xTrgt(2,1),xTrgt(2,3),'*')
plot(xTrgt(3,1),xTrgt(3,3),'*')
...
function error = objectiveFunction(prms, qDes, wt, xMid, xDes)
    ...
    errTar = sum( min( squeeze(sum((xOut - reshape(xMid', 1, size(xMid,2), [])).^2, 2)),[],1));
...
function [c, ceq] = trajConstraint(prms,qDes,xMid)
    ...
    % Target Error
    TarErr= min(squeeze(sum((x - reshape(xMid', 1, size(xMid,2), [])).^2, 2)),[],1);
    
    % End-point Error
    EndErr = (x(end,:) - [0.05, 0.0,0.05]).^2;
