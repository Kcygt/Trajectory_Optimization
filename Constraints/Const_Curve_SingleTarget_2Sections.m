...
zeta = [.5 1 .7 0.5 1 .7];
wn = [1 1 1 1 1 1];
Kp = [70 50 60 70 50 60];
Kd = [35 20 40 35 20 40];

% Weights
wt = [70, 1, 0.02]; % [Target, End, Time]

initPrms = [tspan,zeta, wn, Kp,Kd];

% Initial Condition
[ti, yi] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, tspan,  zeta,wn,Kp,Kd), [0 tspan(2)], zeros(12, 1));
...
[tt, yy] = ode23s(@(t, x) myTwolinkwithprefilter(t, x, qDes, Opt(1:2),  Opt(3:8), Opt(9:14),Opt(15:20),Opt(21:26)), ...
                  [0 Opt(2)], zeros(12, 1));
...
disp(['Kp:   ', num2str(Opt(15:20))])
disp(['Kd:   ', num2str(Opt(21:26))])
...
function error = objectiveFunction(prms, qDes, wt, xMid, xDes)
    ...
    % Simulate the system
    [~, y] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes,prms(1:2),  prms(3:8), prms(9:14),prms(15:20),prms(21:26)), ...
                    [0 prms(2)], x0);
...
function [c, ceq] = trajConstraint(prms,qDes,xMid)
    ...
    % Simulate trajectory
    [~, yy] = ode23s(@(t,x) myTwolinkwithprefilter(t,x,qDes,prms(1:2),  prms(3:8), prms(9:14),prms(15:20),prms(21:26)), ...
                    [0 prms(2)], zeros(12, 1));
